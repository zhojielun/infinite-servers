...
import { fetchInfo, subscribeStatus, buildServers, fetchHistory, toSeries, fetchAvailability, setExpiry, setPurchaseDate, fetchTraffic, fmtBytes, onRefresh } from "./api.js";
...
function useFleet() {
  ...
  useEffect(() => {
    ...
    const loadTraffic = () => fetchTraffic().then((t) => { trafficRef.current = t || {}; rebuild(); }).catch(() => {});
    loadInfo();
    loadTraffic();
    ...
    const unsubRefresh = onRefresh(() => { loadInfo(); loadTraffic(); });
    return () => { unsub && unsub(); unsubRefresh(); };
  }, []);
      ...
      {hasBoth ? (
        <div className="eb-board">
          <div className="eb-legend">
            ...
            <span className="eb-cell eb-green" /><span>{t("expiry.legend.remaining")}</span>
      ...
      <main className="wrap" key={s.name}>
        <div className="dt-head">
          <div className="dt-id">
            ...
            <h1><span className="flag" dangerouslySetInnerHTML={{ __html: s.flag }} />{s.name}</h1>
          ...
          <div className="dt-stats">
            ...
            <div className="dts"><div className="dts-k">{t("dt.network")}</div><div className="dts-v mono">{curNet}{s.traffic && ` | ${t("traffic.upload")}: ${fmtBytes(s.traffic.total_tx)} · ${t("traffic.download")}: ${fmtBytes(s.traffic.total_rx)} · ${t("traffic.total")}: ${fmtBytes(s.traffic.total_tx + s.traffic.total_rx)}`}</div></div>
