...
import { API_BASE, fetchInfo, fetchStatus, subscribeStatus, buildServers, isLoggedIn, login, onRefresh, fetchTraffic, fmtBytes } from "./api.js";
    ...
    <a className={"card anim-rise" + (offline ? " is-offline" : "")} style={{ animationDelay: Math.min(idx || 0, 16) * 28 + "ms" }} href={detailHref(s.name)}>
      ...
      <div className="card-id">
        <div className="card-id-row">
          <span className="flag" dangerouslySetInnerHTML={{ __html: s.flag }} />
          <span className="name">{s.name}</span>
          <span className="region">{s.region}</span>
        </div>
        {(s.ip4 || s.ip6) && (
          <div className="card-ip-row">
      ...
      <div className="specs">
        ...
        {s.traffic && (
          <Spec icon={<Icon.traffic />}>{fmtBytes(s.traffic.total_rx + s.traffic.total_tx)}<i>{t("spec.traffic") || "流量"}</i></Spec>
        )}
      </div>
      ...
      {s.traffic && (
        <div className="card-traffic">
          <span>{t("traffic.upload")}: {fmtBytes(s.traffic.total_tx)}</span>
          <span className="dotsep">·</span>
          <span>{t("traffic.download")}: {fmtBytes(s.traffic.total_rx)}</span>
          <span className="dotsep">·</span>
          <span>{t("traffic.total")}: {fmtBytes(s.traffic.total_rx + s.traffic.total_tx)}</span>
        </div>
      )}
...
function useFleet(authed) {
  ...
  useEffect(() => {
    if (!authed) { setServers(null); setConnError(false); trafficRef.current = {}; return; }
    let info = {};
    let statusMap = {};
    const prevNet = {};
    const rebuild = () => { setServers(buildServers(info, statusMap, prevNet, trafficRef.current)); };

    // Fetch all data in parallel for simultaneous rendering
    Promise.all([fetchData(), fetchTrafficData()]).then(([i]) => { info = i; rebuild(); });

    // Subscribe to status updates (SSE/polling)
    ...
    const unsubRefresh = onRefresh(async (type) => {
...
function App() {
  ...
  const [sortByAlpha, setSortByAlpha] = useState(() => localStorage.getItem("is.sortAlpha") !== "0");
  ...
  const shown = useMemo(() => {
    ...
    if (sortByAlpha) list = [...list].sort((a, b) => a.name.localeCompare(b.name));
    return list;
  }, [servers, filter, query, activeTags, sortByAlpha]);
      ...
      <main className="wrap">
        ...
        <div className="toolbar">
          ...
          <div className="search-group">
            <button
              className={"icon-btn" + (sortByAlpha ? " active" : "")}
              title={t("nav.sortAlpha")}
              onClick={() => { const v = !sortByAlpha; setSortByAlpha(v); localStorage.setItem("is.sortAlpha", v ? "1" : "0"); }}
            >
        ...
        <section className="grid" key={filter + "|" + density + "|" + activeTags.join(",") + "|" + sortByAlpha}>
          {shown.map((s, i) => (
            density === "compact"
