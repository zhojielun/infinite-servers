...
export function flagEmoji(code) {
  const ts = Date.now();
  if (!code || code.length !== 2) return `<img src="https://flagcdn.com/un.svg?v=${ts}" alt="Unknown" class="flag-img">`;
  const cc = code.toUpperCase() === "UK" ? "GB" : code.toUpperCase();
  if (!/^[A-Z]{2}$/.test(cc)) return `<img src="https://flagcdn.com/un.svg?v=${ts}" alt="Unknown" class="flag-img">`;
  return `<img src="https://flagcdn.com/${cc.toLowerCase()}.svg?v=${ts}" alt="${cc}" class="flag-img">`;
}
...
/* ---------- IP masking ----------
   Supports both IPv4 and IPv6 (including shortened IPv6).
   IPv4: mask "x.x.*.*" → "1.2.*.*"
   IPv6: mask "2001:0db8:*:*:*:*:*:*" → "2001:0db8:*:*:*:*:*:*" */
...
function applyIpMask(ip, mask) {
  ...
  if (ip.includes(":") && mask.includes(":")) {
    ...
    const mparts = expandIPv6(mask);
    if (!parts || !mparts || parts.length !== mparts.length) return ip;
  ...
  if (ip.includes(".") && mask.includes(".")) {
    ...
    if (parts.length !== mparts.length) return ip;
    return parts.map((p, i) => mparts[i] === "*" ? "*" : p).join(".");
  }
...
export function buildServers(info, statusMap, prevNet, traffic = {}, now = Date.now()) {
  const out = [];
...
export function subscribeStatus(onStatus, onError) {
  ...
  const trySSE = () => {
    if (stopped) return;
    const token = getAuthToken();
    const url = API_BASE + "status?sse=1";
    const headers = {};
    if (token) headers["Authorization"] = "Bearer " + token;
...
export async function saveServersConfig(servers) {
  const r = await fetch(API_BASE + "api/servers", {
    method: "POST",
    headers: { ...getAuthHeaders(), "Content-Type": "application/json" },
    body: JSON.stringify(servers),
  });
  if (!r.ok) throw new Error("save servers config failed");
  return r.json();
}

export async function fetchServerSettings() {
  return getJSON("api/server-settings");
}

export async function saveServerSettings(settings) {
  const r = await fetch(API_BASE + "api/server-settings", {
    method: "POST",
    headers: { ...getAuthHeaders(), "Content-Type": "application/json" },
    body: JSON.stringify(settings),
  });
  if (!r.ok) throw new Error("save server settings failed");
  return r.json();
}

/* ---------- traffic API ---------- */
export async function fetchTraffic() {
  return getJSON("traffic");
}
...
export function triggerRefresh(type = "all") {
  for (const cb of refreshListeners) {
    try { cb(type); } catch (e) { console.error("Refresh callback error:", e); }
  }
}

/* ---------- Telegram check ---------- */
export async function telegramCheck() {
  const resp = await fetch("/api/telegram-check", {
    method: "POST",
